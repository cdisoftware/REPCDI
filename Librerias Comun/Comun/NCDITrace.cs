...
namespace CDI.Comun
{
    ...
    public class NCDITrace
    {
        ...
        public enum Tipo
        {
            ...
            /// <summary>
            /// Cuando se quiere guardar el proceso de tiempo base de datos
            /// </summary>
            Time
        }
        ...
        private static void EscribirDD(MethodBase Nombre, string pMensaje, Tipo pTipo)
        {
            try
            {
                ...
                if (pTipo == Tipo.Time)
                {
                    string sUbicacionE = DirLog + "\\" + fecha + @"\" + "Time" + DateTime.Now.ToString("HH") + ".txt";
                    sUbicacionE = sUbicacionE.Replace("\0", "");
                    using (StreamWriter w2 = new StreamWriter(sUbicacionE, true))
                    {
                        w2.WriteLine("-- " + DateTime.Now.ToString("HHmm") + " --- " + pTipo.ToString() + " --- Metodo: " + Nombre.Name + " --- Clase: " + Nombre.ReflectedType.FullName + " ----");
                        w2.WriteLine(pMensaje + " ejecucion " + DateTime.Now.ToString("T"));
                        w2.Flush();
                        w2.Close();
                    }
                }
